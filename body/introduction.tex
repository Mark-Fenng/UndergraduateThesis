...
使得启发式算法得以普遍应用[7-9]。查询优化器有时将遗传算法[?]或随机算法应用到大的连
接查询中。在边界情况下，这些启发式算法可能崩溃，从而导致较差的连接计划。
