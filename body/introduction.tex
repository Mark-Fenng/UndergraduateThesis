...
\end{figure}
\section{本章小结}
在这一章节中，我们提出了一种基于强化学习的方法来对SPARQL查询中基本图模式查询进行优化，该方法能够优化基本图模式中的三元组模式中间查询结果的连接顺序，从而到达提高查询效率的目的。我们首先对SPARQL的基本图模式优化问题进行了详细的描述，展现了较大的优化空间，确定了优化问题的输出输出。之后，我们从马尔可夫决策过程的五个要素（状态，动作，策略，奖励，初始状态）开始，分别详细得介绍了状态、初始状态、动作的定义和特征化方法，然后通过举例的方式描述了状态之间的转换方式，最后介绍了奖励值的计算方法。
