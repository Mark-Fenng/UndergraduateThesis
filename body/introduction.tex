...
为了便于RDF数据的检索，万维网联盟(简称为W3C)提出了一种形式化的查询语言SPARQL，它是目前最广泛使用的查询语言之一。类似于SQL，SPARQL也支持多种查询模式， 例如SELECT、ASK、DESCRIBE、CONSTRUCT的多种查询格式。其中，SELECT查询格式是一种最常用的标准查询，它以标准的XML格式返回查询结果，这个结果是匹配到的RDF图数据。ASK 查询返回结果不包含具体的数据，而是 yes 或 no的判断结果。DESCRIBE 查询是为了提取本体和实例数据的一部分，返回一个包含匹配图模式的结点相关信息的图形。
...
强化学习(Reinforcement learning)\cite{RLIntroduction}是机器学习(machine learning)的其中一种方法，这种方法的主要思路是让学习者(agent)通过不断获得环境的反馈来学习。强化学习方法最早是来源于行为主义理论，这个理论是关于个体如何在接受环境反馈的刺激下，逐渐形成对环境刺激的预期，最终学会如何使利益最大化。例如当婴儿开始学习走路时，他刚开始并不能掌握平衡，并不会直接学会走路，但他通过与周围环境的交互获得反馈，例如摔倒会给他带来疼痛感（惩罚），顺利走出第一步保持了平衡（奖励），婴儿就会知道每个动作的后果（获得惩罚或奖励），从而逐渐选择奖励最多的动作，直到完全学会走路（奖励最大化）。当我们从计算机科学的角度看待这种方法时，我们可以将其解释为一个函数。该函数尝试使奖励累积结果最大化，而无需告知必须采取何种行动。
