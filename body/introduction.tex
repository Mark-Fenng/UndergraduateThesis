...
\begin{enumerate}
    ...
    \item 每一个SPARQL查询都有不同的基本图模式(BGP)，即如果有两个不同的SPARQL查询，则它们的三元组模式和总个数都不尽相同。而对于强化学习算法来说，其数据输入（状态的特征编码）的维度需要维持衡定，所以如何将个数变化的三元组模式集特征化为一个长度固定的编码是比较困难的。
\end{enumerate}
...
根据以上的定义，我们首先对输入的SPARQL查询中的三元组模式进行第一次映射。对于一个三元组模式，如果它是一个类型三元组模式，则将其映射为它的客体(subject)；如果它是一个属性三元组模式，则将其映射为它的属性(property)。这样做映射的目的是...

例如，三元组模式$T_1$：" ?X rdf:type ub:GraduateStudent " 是一个类型三元组模式，所以将其映射为ub:GraduateStudent。三元组模式$T_2$：" ?X ub:memberOf ?Z "是一个属性三元组模式，所以将其映射为ub:memberOf。

为了使不同的查询都能有相同长度的编码，我们需要在初始时获得整个图数据中所有的类型值和属性，即图中所有的rdf:type对应的客体值和所有的属性值，这个可以通过在导入RDF数据时进行统计和记录即可获得。然后，对于这些统计值进行简单的计数排序编码。之后将对三元组模式进行第二次映射，即将其第一次映射编码的结果转换为其对应的计数编码。

例如...
\subsection{动作的定义和特征化}[action]
$A$表示有限动作集（动作空间），对任意时刻$t$的动作$A_t$，都满足$A_t \in A$。
在我们的优化问题中，在某个连接状态下，所有可能的连接动作构成了有限动作集。在t时刻，其连接状态为$S_t$，则此时，所有可能的连接动作就是整个动作空间$A$，而学习者会从动作集中选择一个动作$A_i$去实际执行。

以查询\ref{sparql1}为例子，这个查询中包括6个三元组模式(Triple)：$T_1, T_2, T_3, T_4, T_5, T_6$。

以下过程将展示动作集和动作是如何随时间变化的：
\begin{enumerate}
    \item 初始时的状态$S_0=\phi$，表示所有的三元组模式都没有连接，此时，可以从剩余的6个三元组模式中任选一个开始执行实际查询，其动作集大小为6，假设学习者选择首先执行三元组模式$T_1$得到其中间查询结果，则它选择的动作是与$T_1$的中间查询结果做连接操作。
    \item 如果在$t_1$时刻，已经得到了三元组模式$T_1$的查询中间结果，则此时的状态可以表示为$S_{t1} = T_1$； 
    \item 如果在$t_2$时刻，三元组模式$T_1$的查询中间结果与$T_3$的查询中间结果进行了连接操作，则此时的状态可以表示为$S_{t2} = T_1 \Join T_3$；
    \item 如果在$t_3$时刻，前面的连接中间结果又与三元组模式$T_2$的查询中间结果进行了连接操作，则此时的状态可以表示为$S_{t3} = T_1 \Join T_3 \Join T_2$；
    \item 以此类推，如果最终查询结束时，这6个三元组模式(Triple)查询中间结果的连接顺序是$T_1 \Join T_3 \Join T_2 \Join T_5 \Join T_4 \Join T_6$，则在查询处理结束的时刻$t_6$时，状态的表示为$S_{t6} = T_1 \Join T_3 \Join T_2 \Join T_5 \Join T_4 \Join T_6$。
